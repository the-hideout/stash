    ...
    async execute(interaction) {
        ...
        for (const item of searchedItems) {
            for (const barter of barters) {
                if (barter.requiredItems.some(r => r.item === item.id)) {
                    matchedBarters.push(barter.id);
                    continue;
                }
                if (barter.rewardItems.some(r => r.item === item.id)) {
                    matchedBarters.push(barter.id);
                    continue;
                }
            }
        ...
        if (matchedBarters.length > MAX_BARTERS) {
            ...
            for (let i = MAX_BARTERS; i < matchedBarters.length; i = i + 1) {
                ...
                const bitemname = `[${rewardItem.name}](${rewardItem.link}) (${trader.name} LL${barter.minTraderLevel})`;
